...
if(isset($_POST["checkout"])){
  ...
  $result = mysqli_query($conn,"SELECT c.cart_id,c.quantity,p.pro_id,p.pro_name,p.pro_price,p.pro_image from cart 
  as c INNER JOIN product as p on p.pro_id=c.pro_id where c.user_id='$user'"); 
    while($row = mysqli_fetch_array($result)){
      ...
      $ins=mysqli_query($conn,"INSERT INTO orderdb (pro_id,pro_image,order_date,user_id,totalprice,status,quantity)
       values('$id','$image',now(),'$user','$price','placed','$quantity')");
      if($ins){
        $flag=1;
      }
      }
...
$sqla = mysqli_query($conn,"SELECT * from orderdb where user_id='$user'" );
...
$sqll=mysqli_query($conn,"SELECT * FROM user where user_id='$user'");
$val=mysqli_fetch_array($sqll);

if($val['address']==null){
  echo '<div class="font-size-20 text-danger font-baloo">PLEASE FILL THE DELIVERY DETAILS!</div>';?>
  <div style="float:right;">
  <input type="button" value="EDIT PROFILE" onclick="document.location.href='editacc.php'" 
