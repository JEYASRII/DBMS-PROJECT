...
$result = mysqli_query($conn,"SELECT c.cart_id,c.quantity,p.pro_id,p.pro_name,p.pro_price,p.pro_image,p.stock from 
cart as c INNER JOIN product as p on p.pro_id=c.pro_id where c.user_id='$user_id'"); 
...
?>
  ...
  <input type='hidden' name='price' value="<?php echo $row["pro_price"];?>">
...
if(isset($_POST["update"])){
  ...
  if($flag==0){
  mysqli_query($conn,"UPDATE cart set quantity='$quantity',price='$price' where cart_id='".$cart_id."'");
  }
...
<input type="submit" name="checkout" value="CHECKOUT" class="btn btn-warning font-size-12" >
</form>
<br>
</div>
